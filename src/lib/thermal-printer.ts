...
const formatDateSafe = (dateString: string): Date => {
  ...
  // Se for apenas data (YYYY-MM-DD), adiciona horário para evitar timezone issues
...
export const printThermalReceipt = async (data: ThermalPrintData): Promise<void> => {
  try {
    ...
    const displayDate = data.type === 'quote' 
      ? (data.quoteDate ? formatDateSafe(data.quoteDate) : new Date())
      : (data.saleDate ? formatDateSafe(data.saleDate) : new Date());
    ...
    <html>
      ...
      <body>
        <div class="receipt">
          ...
          <div class="center">${data.type === 'quote' ? (data.quoteDate ? formatDateSafe(data.quoteDate).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR')) : (data.saleDate ? formatDateSafe(data.saleDate).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR'))}</div>
          ...
          <div class="center">${data.type === 'quote' ? (data.quoteDate ? formatDateSafe(data.quoteDate).toLocaleString('pt-BR') : new Date().toLocaleString('pt-BR')) : (data.saleDate ? formatDateSafe(data.saleDate).toLocaleString('pt-BR') : new Date().toLocaleString('pt-BR'))}</div>
        </div>
      </body>
    </html>
  `);
